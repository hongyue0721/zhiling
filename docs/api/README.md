...
当前已发布的知径业务接口是需要正式 Session 的精选学习地图目录与详情读取；前端接入、字段语义和示例见[精选学习地图前端接入契约](featured-learning-maps.md)。外部供应方协议不写入知径 OpenAPI，知乎接口单独记录在[知乎开放平台外部契约](zhihu-open-platform.md)。
