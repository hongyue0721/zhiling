...
| ------- | ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------ | ---------- | ------------------------------------------------------------- | --------------------------------------------------------------- |
| ZH-001  | 知乎站内搜索 HTTP → `SourceSearchAccess` | `GET https://developer.zhihu.com/api/v1/content/zhihu_search`；官方 `zhihu` Skill 0.2.1/2026-07-16 契约；ContentType `Answer/Article/Question` 和 AuthorityLevel `1..4` 的封闭转换；已观察的 `AuthorSignature` 可选附加字段；已观察的空/空白 `AuthorName` 规范化为 `匿名`（信息损失：原作者身份不可恢复）。适配器版本 `zhihu-http-2026-07-16-v3`。当前官方资料没有独立“正式接口/比赛接口”端点或 DTO，故只实现该统一正式契约；赛事若另行提供契约，必须新增兼容项，不伪造第二套实现 | 必要字段缺失（含 `AuthorName` 缺字段或非字符串）、严格 JSON、URL HTTPS/知乎主机、未知枚举（包括 `Zvideo`/`Pin`）、HTTP 状态或业务 `Code` 不符时返回 `ExternalProviderError(protocol_error)` 或稳定错误类别；空/空白 `AuthorName` 不构成失败；空列表仅在合法成功响应时允许 | 外部适配模块 | 2026-09-02 | 官方端点/字段或枚举变化，或新增赛事契约并完成新版本适配和迁移 | 已实现；2026-09-04/07 已观察成功搜索及空 AuthorName            |
...
| GEN-001 | 生成任务阶段 → 外部错误重试              | ADR-0004 的每个外部阶段对 `rate_limited`、`temporarily_unavailable`、`timeout` 最多重试 2 次；适配器不自行重试；供应方未返回 `Retry-After` 时，模型调用按 5 秒、10 秒退避，来源搜索按 250 毫秒、500 毫秒退避，更长的供应方等待时间优先                                                                              | `invalid_request`、`authentication_failed`、`quota_exhausted`、`protocol_error` 不按瞬时外部失败重试；模型协议错误使用独立的受限恢复预算；第三方正文不跨边界                                     | 地图生成模块 | 2026-09-02 | 生成策略或供应方错误契约变更                                  | 已接受                                                          |
...
2026-09-04 与 2026-09-07 已在受控生产配置下取得成功搜索响应；后者确认部分条目 `AuthorName` 为空或纯空白。成功空结果、限流、配额和直答结构化成功的完整在线样本仍未取得。契约测试验证脱敏 fixtures、严格解析和错误转换，但不能替代完整生成闭环验收。不得把文档样本或无密钥失败探针宣称为在线成功验收。
