...
2026-09-02 对已完成的 C1、C2 和 D2 进行了定向功能复验，未修改代码、测试或数据库文件：

- C1 身份与会话、C2 学习目录：80 项通过，包括 29 项契约、23 项单元、16 项配置、6 项身份 PostgreSQL 集成和 6 项目录 PostgreSQL 集成测试；
- D2 学习验证与进度：16 项通过，包括 5 项领域、4 项公共契约、2 项 HTTP 契约和 5 项 PostgreSQL 集成测试；
- 合计 96 项功能测试全部通过，集成测试使用 `127.0.0.1:5433/zhijing_test` 专用 PostgreSQL；
- 覆盖注册/验证/登录/会话失效、地图目录和关系授权、四类判题、幂等提交、最佳成绩单调更新、80% 完成边界、进度恢复和题目版本隔离。

本次记录不包含前端完整用户闭环、C3 外部适配器、D1 地图生成、E2 私人结课报告或 F 端到端验收；也未执行安全专项审计。上述范围仍以对应 Issue 的阻塞状态为准。
