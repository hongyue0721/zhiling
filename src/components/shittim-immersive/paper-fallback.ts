...
export type PaperRenderer = { setMode(mode: PaperMode): void; setPaused(paused: boolean): void; burst(): void; destroy(): void; renderer: "three-webgl" | "canvas2d" | "static" };
/** Immediate fallback: same folio shape, no images or timers pretending to be task progress. */
export function mountPaperFallback(host: HTMLElement, initial: PaperMode): PaperRenderer {
  const canvas = document.createElement("canvas");
  canvas.setAttribute("aria-hidden", "true");
  canvas.style.cssText = "display:block;width:100%;height:100%;pointer-events:none";
  host.append(canvas);
  const ctx = canvas.getContext("2d");
  if (!ctx) return { renderer: "static", setMode() {}, setPaused() {}, burst() {}, destroy() { canvas.remove(); } };
  host.dataset.renderer = "canvas2d";
  const seeds = particleSeeds(2400);
  const media = matchMedia("(prefers-reduced-motion: reduce)");
  let mode = initial, paused = false, destroyed = false, raf = 0, width = 1, height = 1;
  let time = 0, last = performance.now(), burst = 0, mx = -10, my = -10;
  ...
  function draw(now = performance.now()) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      let px = positions[j], py = positions[j + 1];
      const distance = Math.hypot(px - mx, py - my);
      const force = moving ? Math.exp(-distance * distance * 7) * .12 : 0;
      px += (px - mx) * force + Math.sin(seeds[k] * 65) * burst * .3;
      py += (py - my) * force + Math.cos(seeds[k + 1] * 40) * burst * .35;
      const perspective = 3.6 / (3.6 + positions[j + 2]);
      context.fillStyle = i % 7 === 0 ? palette.gold : palette.ink;
      context.globalAlpha = p.alpha * (ambient ? 1 : .84);
      context.beginPath();
      context.arc(width * .5 + px * scale * perspective, height * .56 - py * scale * perspective, Math.max(.4, p.size * .63 * perspective), 0, Math.PI * 2);
      context.fill();
    }
    initialized = true;
  ...
  function wake() { cancelAnimationFrame(raf); last = performance.now(); draw(); }
  function resize() {
  ...
  const onPointer = (event: PointerEvent) => {
    ...
    mx = (event.clientX - r.left - width / 2) / scale; my = -(event.clientY - r.top - height * .56) / scale;
  };
  const leave = () => { mx = -10; my = -10; };
  const ro = new ResizeObserver(resize); ro.observe(host);
  document.addEventListener("pointermove", onPointer, { passive: true });
  document.addEventListener("pointerleave", leave); document.addEventListener("visibilitychange", wake);
  window.addEventListener("shittim:theme", wake); media.addEventListener("change", wake); resize();
  return {
    renderer: "canvas2d", setMode(value) { mode = value; wake(); },
    setPaused(value) { paused = value; wake(); }, burst() { if (!paused && !media.matches) { burst = 1; wake(); } },
    destroy() { destroyed = true; cancelAnimationFrame(raf); ro.disconnect();
      document.removeEventListener("pointermove", onPointer); document.removeEventListener("pointerleave", leave);
      document.removeEventListener("visibilitychange", wake); window.removeEventListener("shittim:theme", wake);
      media.removeEventListener("change", wake); canvas.remove(); },
  };
}
