...
export class ProgressConnectionError extends Error {
  constructor(
    ...
    super(message);
    this.name = "ProgressConnectionError";
  }
}
...
export async function followGeneration(options: FollowOptions): Promise<void> {
  ...
  while (!signal.aborted) {
    ...
    const touch = () => {
      clearTimeout(timer);
      timer = setTimeout(abort, options.idleMs ?? 45_000);
    };
    ...
    try {
      ...
      if (!response.ok)
        throw new ProgressConnectionError(
          "生成进度连接暂不可用",
          response.status,
        );
      ...
      while (!signal.aborted) {
        ...
        for (const event of decoder.push(value)) {
          ...
          if (isTerminal(event.type) || isTerminal(eventStage(event) ?? "")) {
            terminal = true;
            break;
          }
        }
      ...
      if (
        error instanceof ProgressConnectionError &&
    ...
    try {
      ...
      if (signal.aborted) return;
      if (snapshot.taskId !== taskId || snapshot.sequence < sequence)
        throw new ProgressConnectionError("任务快照尚未追上进度流");
    ...
    if (reconnects > (options.maxReconnects ?? 6))
      throw new ProgressConnectionError(
        "进度连接暂时中断。任务可能仍在执行，请重新连接原任务。",
      );
    options.onConnection("reconnecting", reconnects);
    await (options.sleep ?? abortableDelay)(
      Math.min(8_000, 700 * 2 ** (reconnects - 1)),
      signal,
    );
  }
}
