...
export function mountUniverse(
  ...
  if (!forceFallback) {
    void Promise.all([import("three"), import("./paper-three")])
      .then(([THREE, { mountPaperThree }]) => {
        if (destroyed) return;
        try {
          const candidate = mountPaperThree(host, mode, THREE, () => {
            active.destroy();
            active = mountPaperFallback(host, mode);
      ...
      .catch(() => {
        /* Chunk/network failure must never remove the usable page. */
      });
  }
  return {
    get renderer() {
    ...
    setMode(value) {
      mode = value;
      active.setMode(value);
    },
    setPaused(value) {
      paused = value;
      active.setPaused(value);
    },
    burst() {
      active.burst();
    },
