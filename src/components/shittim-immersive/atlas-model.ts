...
export type AtlasEdge = Readonly<{
  nodeId: string;
  prerequisiteNodeId: string;
}>;
export type Position = { id: string; x: number; y: number; depth: number };
export type Answerable = Readonly<{
  questionId: string;
  type: string;
  options: readonly { optionId: string; side?: string }[];
}>;
...
export function layoutAtlas(
  ...
  for (const e of edges) {
    ...
    if (
      !ids.has(e.nodeId) ||
      !ids.has(e.prerequisiteNodeId) ||
      e.nodeId === e.prerequisiteNodeId ||
      seen.has(key)
    )
      continue;
    seen.add(key);
    indegree.set(e.nodeId, indegree.get(e.nodeId)! + 1);
  ...
  const ordered = [...nodes].sort(
    (a, b) => depth.get(a.nodeId)! - depth.get(b.nodeId)!,
  );
  const columns = mobile
    ? Math.min(2, nodes.length || 1)
    : Math.min(4, Math.max(1, Math.ceil(nodes.length / 2)));
  ...
  return ordered.map((n, i) => {
    ...
    return {
      id: n.nodeId,
      depth: depth.get(n.nodeId)!,
      x: columns === 1 ? 50 : 13 + (column * 74) / (columns - 1),
      y:
        rows === 1
          ? 48
          : 18 + (row * 62) / (rows - 1) + (mobile ? 0 : column % 2 ? -9 : 5),
    };
  });
}
export function curvePath(a: Position, b: Position): string {
  const ax = a.x * 10,
    ...
    bx = b.x * 10,
    by = b.y * 6;
  return `M ${ax} ${ay} C ${ax + (bx - ax) * 0.45} ${ay}, ${bx - (bx - ax) * 0.45} ${by}, ${bx} ${by}`;
}
export function isAnswered(q: Answerable, picks: Picks, pairs: Pairs): boolean {
  if (q.type === "matching") {
    const left = q.options.filter((o) => o.side === "left"),
      right = new Set(
        q.options.filter((o) => o.side === "right").map((o) => o.optionId),
      );
    const values = left.map((o) => pairs[q.questionId]?.[o.optionId] ?? "");
    return (
      left.length > 0 &&
      values.every((v) => right.has(v)) &&
      new Set(values).size === values.length
    );
  }
  const selected = picks[q.questionId] ?? [],
    allowed = new Set(q.options.map((o) => o.optionId));
  return (
    selected.length > 0 &&
    selected.every((id) => allowed.has(id)) &&
    new Set(selected).size === selected.length &&
    (q.type === "multiple_choice" || selected.length === 1)
  );
}
export function buildAnswers(
  questions: readonly Answerable[],
  picks: Picks,
  pairs: Pairs,
) {
  if (!questions.length || !questions.every((q) => isAnswered(q, picks, pairs)))
    return null;
    ...
    q.type === "matching"
      ? {
          ...
          matches: q.options
            ...
            .map((o) => ({
              leftOptionId: o.optionId,
              rightOptionId: pairs[q.questionId][o.optionId],
            })),
        }
      : {
          questionId: q.questionId,
          selectedOptionIds: [...picks[q.questionId]],
        },
  );
}
export function nextNodeId(
  ...
  return (
    nodes.find((n) => n.nodeId !== current && !completed.has(n.nodeId))
      ?.nodeId ?? null
  );
}
export function clampZoom(value: number) {
  return Math.max(0.65, Math.min(1.9, value));
}
