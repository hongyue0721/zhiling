...
export function readResult(value: unknown): TaskResult | null {
  ...
  return r &&
    text(r.mapId) &&
    text(r.versionId) &&
    text(r.learningRelationshipId)
    ? {
        mapId: text(r.mapId),
        versionId: text(r.versionId),
        learningRelationshipId: text(r.learningRelationshipId),
      }
    : null;
}
export function readSnapshot(value: unknown): TaskSnapshot {
  ...
  return {
    ...
    stage: text(r.stage) || text(r.status),
    sequence: Number(r.sequence),
    createdAt: text(r.createdAt),
    updatedAt: text(r.updatedAt),
    deadlineAt: text(r.deadlineAt),
...
export function parseSseBlock(block: string): TaskEvent | null {
  ...
  try {
    ...
    if (
      ...
      return null;
    if (!["snapshot", "progress", "succeeded", "failed"].includes(text(e.type)))
      return null;
...
export function acceptsEvent(
  ...
  return (
    ...
    (event.sequence > sequence ||
      (event.type === "snapshot" && event.sequence === sequence))
  );
}
