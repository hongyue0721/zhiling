...
export function mountPaperThree(
  ...
  const alphaAttribute = new THREE.BufferAttribute(alpha, 1).setUsage(
    THREE.DynamicDrawUsage,
  );
  ...
  const paperGeometry = new THREE.PlaneGeometry(0.027, 0.048);
  ...
  function theme() {
    ...
    material.uniforms.uInk.value.set(palette.ink);
    material.uniforms.uGold.value.set(palette.gold);
    paperMaterial.color.set(palette.gold);
    wake();
  }
  function draw(now = performance.now()) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const dx = p.x - mx,
        dy = p.y - my;
      const force = moving ? Math.exp(-(dx * dx + dy * dy) * 7) * 0.12 : 0;
      const x = p.x + dx * force + Math.sin(seeds[k] * 65) * burst * 0.3;
      const y = p.y + dy * force + Math.cos(seeds[k + 1] * 40) * burst * 0.35;
    ...
    for (let i = 0; i < 24; i++) {
      ...
      dummy.position.set(
        (seeds[k] - 0.5) * 5.8,
        (seeds[k + 1] - 0.5) * 3.4 + Math.sin(time * 0.17 + i) * 0.1,
        -0.6 + seeds[k + 2] * 0.6,
      );
      dummy.rotation.set(time * 0.16 + i, time * 0.11 + i * 1.8, seeds[k] * 7);
      dummy.scale.setScalar(ambient ? 0.7 : 1 + burst * 1.5);
      dummy.updateMatrix();
  ...
  function resize() {
    ...
    camera.position.z =
      height / (2 * scale * Math.tan(THREE.MathUtils.degToRad(20)));
    camera.position.y = (height * 0.06) / scale;
  ...
  const onPointer = (e: PointerEvent) => {
    ...
    mx = (e.clientX - r.left - width / 2) / scale;
    my = -(e.clientY - r.top - height * 0.56) / scale;
  };
  const leave = () => {
    mx = -10;
    my = -10;
  ...
  window.addEventListener("shittim:theme", theme);
  ...
  return {
    ...
    destroy() {
      ...
      window.removeEventListener("shittim:theme", theme);
